<template>
  <div class="tody">
     <section>
       <van-callapse v-for="(item,index) in dayComputed.habits" :key="item.id" @change="change" v-model="today.active">
         <van-collapse-item :name = 'index'>
           <p slot="title"><icon name="time">{{item.title}}</icon></p>
            <aside v-for="ele in item.habits" :key="ele.id" @click="finish(ele.id)">
              <Circles radius="3.5rem" v-if="!!ele.habitLog.data.find(itm=>(item.id === days))" :activeColor="!!ele.habitLog.date.find(item =>(item.id === days)).isFinished ? ele.color : '#fff'">
                <icon :name="ele.iconName" slot="icon"></icon>
              </Circles>
            </aside>
         </van-collapse-item>
       </van-callapse>
     </section>
     <ClockPopup :show="show" @save="saveLogs" @hide='hide'></ClockPopup>
  </div>
</template>
...
import {HabitList as HabitListState,TimeSlotList, HabitList} from '@/store/state';
import _ from '@/utils'
...
export default class Home extends Vue {
  ...
  @Mutation private saveLog!: (playload:{id:number;daysId:number;message:string})=> void;
